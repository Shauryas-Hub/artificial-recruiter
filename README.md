...
│   ├── io_utils.py                ← streaming JSONL reader (gzip support)
...
│   ├── test_edge_cases.py
│   └── test_integration.py
├── artifacts/                     ← generated by precompute.py (gitignore large files)
...
└── validate_submission.py         ← official hackathon validator (copy)
```
...
## Sandbox Deployment (HuggingFace Spaces)

1. Create a new Space at [huggingface.co/new-space](https://huggingface.co/new-space)
2. Choose **Streamlit** SDK
3. Upload all project files
4. The `sandbox/app.py` will be the entry point
5. Paste the Space URL into `submission_metadata.yaml → sandbox_link`
...
| Requirement | Status |
|---|---|
| ≤5 min ranking step | ✅ rank.py: <10s on CPU |
| CPU only | ✅ no GPU imports in rank.py |
| No network during ranking | ✅ no HTTP calls, no model.encode() in rank.py |
| ≤16 GB RAM | ✅ parquet + pandas well within limit |
| Exactly 100 rows | ✅ validated by csv_writer.py |
| Score non-increasing | ✅ enforced in scorer.py + csv_writer.py |
| Honeypot rate ≤10% | ✅ honeypots hard-dropped before scoring |
| UTF-8 CSV | ✅ enforced in csv_writer.py |
| Unique candidate IDs and ranks | ✅ validated pre-write |
| Varied, specific, non-hallucinated reasoning | ✅ reasoning.py from feature columns |
